//! Tabs in the large size.
//!
//! `TabsSize::Large` changes the tab row height; the layout reads the actual
//! height back through `tabs.height()`, so the constraint cannot drift.
